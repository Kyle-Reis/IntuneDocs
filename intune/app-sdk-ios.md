...
4. Enable ADAL single sign-on (SSO) by adding `com.microsoft.adalcache` to the keychain access groups. By default, the ADAL library will use this keychain group for the shared ADAL cache. If your app(s) override this default, you should ensure the custom access group is added here instead.

5. If your app(s) use a custom keychain group to replace `com.microsoft.adalcache`, specify that in the Info.plist file under IntuneMAMSettings, by using the key `ADALCacheKeychainGroupOverride`.
